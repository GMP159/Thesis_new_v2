"""
FIXED Visualization script - Samples from ALL domains
"""
...
#     """Infer number of classes from checkpoint."""#for non-DDP only
#     checkpoint = torch.load(checkpoint_path, map_location='cpu',weights_only=False)

#     if 'classification_head.classifier.weight' in checkpoint['model_state_dict']:
#         weight_shape = checkpoint['model_state_dict']['classification_head.classifier.weight'].shape
...
def infer_num_classes(checkpoint_path):
    ...
    return 8  # Default to 8 (your training has 8 domains)



# def load_checkpoint(checkpoint_path):
#     """Load model from checkpoint."""#for non-DDP only
...
def sample_from_each_domain(data_loader, domain_mapping, samples_per_domain=2):
    """Sample patches from each domain. FIXED to get diverse samples!"""
    print("\nSampling from each domain...")

    # Collect all samples by domain
    ...
    for patches, labels in data_loader:
        ...
        # Check if we have enough samples from all domains
...
def analyze_embeddings(model, data_loader, device, save_path, domain_mapping, n_samples=800):
...
def main():
    # Configuration
    checkpoint_dir = r'D:\Thesis_new_v2\outputs\checkpoints\pretrain'
    ...
    print("\n" + "="*80)
    print("VISUALIZING PRETRAINED MODEL - FIXED VERSION")
    print("="*80)
    ...
    # Visualize (FIXED to sample from all domains)
    visualize_reconstructions(
        model, val_loader, device,
        os.path.join(output_dir, 'reconstructions_fixed.png'),
        domain_mapping,
        n_samples=8
    )

    # t-SNE (processes all batches)
    analyze_embeddings(
        model, val_loader, device,
        os.path.join(output_dir, 'embeddings_tsne_fixed.png'),
        domain_mapping,
        n_samples=400  # Reduced from 800 for faster execution
    )
    ...
    print("  2. reconstructions_fixed.png - NOW WITH ALL 4 DOMAINS!")
    print("  3. embeddings_tsne_fixed.png - NOW WITH ALL 4 DOMAINS!")
    print("="*80)
